				...
				<div id="characterSheets" class:active={characterSheetsExpanded} class:dark={$characterSheets == "dark"}>
					<div class="tab-container">
						...
						{#each $sheetModules[$characters[$currentCharacter].module.id].info.sheets as sheet, i}
							<button style="margin-left: {i > 0 ? 2 : 0}px; margin-right: {i < $sheetModules[$characters[$currentCharacter].module.id].info.sheets.length - 1 ? 2 : 0}px;" class="sheet-button" class:active={currentCharacterSheet == i} on:click="{() => {currentCharacterSheet = i}}">{sheet.displayName}</button>
						{/each}
