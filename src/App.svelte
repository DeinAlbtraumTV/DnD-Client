<script>
	...
	import { characters, socket, sheetModules } from "./stores/stateStore"
	...
	let newCharacterModule = $state("dnd_5e_builtin")
	...
	$effect(() => {
		...
		sheetAnchor.after(sheetElement)
	...
	function checkIfVersionMatchesServer(requiredVersion) {
		...
		for (let i = 0; i < requiredVersionParts.length; i++) {
			...
			if (Number.isNaN(requiredVersionPart) || Number.isNaN(clientVersionPart)) {
				notifications.push({
					type: NotificationType.ERROR,
...
<main>
	...
	<Sidebar>
		<div class="flex-container">
			...
			<div id="sheet-container">
				<div role="button" tabindex=0 id="sheetExpander" class:expanded={characterSheetsExpanded} onkeydown={sheetExpanderKeydown} onclick={toggleCharacterSheet}>
				...
				<div id="characterSheets" class:active={characterSheetsExpanded} class:dark={$characterSheets == "dark"}>
					<div class="tab-container">
						<select name="character" id="characterSelect" bind:this="{characterSelect}" onchange={showCharacterPopup} bind:value="{$currentCharacter}" onfocus={characterSelectFocused}>
							{#each Object.keys($characters) as name (name)}
								{#if name != "version"}
						...
						{#if $characters[$currentCharacter] && $sheetModules[$characters[$currentCharacter].module.id]}
							{#each $sheetModules[$characters[$currentCharacter].module.id].info.sheets as sheet, i (sheet)}
								<button style="margin-left: {i > 0 ? 2 : 0}px; margin-right: {i < $sheetModules[$characters[$currentCharacter].module.id].info.sheets.length - 1 ? 2 : 0}px;" class="sheet-button" class:active={currentCharacterSheet == i} onclick={() => {currentCharacterSheet = i}}>{sheet}</button>
							{/each}
						{/if}
	...
	<div class="popup-container" class:active="{showNewCharacterPopup}">
		<div class="popup-content">
			<div class="flex-column">
				<button aria-label="Close Popup" id="closePopup-button" onclick={() => {showNewCharacterPopup = false; characterSelect.value = prevSelected; currentCharacter.set(prevSelected)}}><i class="far fa-times-circle"></i></button>
			</div>
			...
			<select class="input boxShadow select" id="characterModule-input" bind:value="{newCharacterModule}">
				{#each Object.entries($sheetModules) as entry (entry)}
	...
	<div class="popup-container" class:active="{showCharacterDeletionPopup}">
		<div class="popup-content">
			<div class="flex-column">
				<button aria-label="Close Popup" id="closePopup-button" onclick={() => {showCharacterDeletionPopup = false;}}><i class="far fa-times-circle"></i></button>
			</div>
...
<style>
	...
	#charactersheet-container.visible {
		display: block;
	}
