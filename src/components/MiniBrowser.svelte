<script>
    ...
    import { socket, session } from "../stores/stateStore"
    ...
    let tabTopTabs = $state([])
    let webviewTabs = $state([])
    ...
    function changeToTab(event) {
        ...
        if (index < 0 || index >= tabTopTabs.length) {
            ...
            document.getElementById("urlInput").value = tabTopTabs[index].url
        }

        currentTab = index
    }

    function changeToTabKeyboard(event) {
        if (event.key == "Enter") {
            tabContextMenuVisible = false
            let target = event.currentTarget
    ...
    function addTab() {
        ...
        document.getElementById("urlInput").value = tabTopTabs[currentTab].url

        $localStorageStore.tabs = JSON.stringify(tabTopTabs)
    }

    function addTabWithUrl(event) {
    ...
    function openUrlInTabAtEnd(url) {
        let tab = {
            title: "Empty Tab",
            url: url,
            id: (tabTopTabs.length == 0 ? 0 : tabTopTabs[tabTopTabs.length - 1].id + 1)
    ...
    function closeTab(event) {
        ...
        if (index >= 1)
            document.getElementById("urlInput").value = tabTopTabs[index - 1].url
        else
            document.getElementById("urlInput").value = ""
    ...
    function closeTabKeyboard(event) {
        if (event.key == "Enter") {
            event.stopPropagation()
            let target = event.currentTarget;
    ...
    function webviewLoaded(event) {
        let target = event.target
        let index = target.getAttribute("data-index")

        let title = target.getTitle()
    ...
    function toggleTabContextMenu(event) {
    ...
    function duplicateTab() {
        if (tabContextMenuTarget > -1) {
            ...
            document.getElementById("urlInput").value = tabTopTabs[tabContextMenuTarget].url

            $localStorageStore.tabs = JSON.stringify(tabTopTabs)
        }
    }

    function injectAdBlockCss(event) {
        let target = event.target
        let index = target.getAttribute("data-index")
